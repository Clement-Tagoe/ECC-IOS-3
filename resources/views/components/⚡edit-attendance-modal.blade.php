...
new class extends Component
{
    ...
    public function saveAttendance(): void
    {
        AttendanceModel::updateOrCreate(
            ['monitoring_staff_id' => $this->editingStaffId, 'date' => $this->editingDate],
            ['status' => $this->editingStatus, 'notes' => $this->editingNotes]
        );

        $this->dispatch('close-modal', id: 'edit-monitoring-staff-attendance');

        // Browser-level event carrying the actual data — no server round trip needed to consume it
        $this->dispatch('attendance-cell-updated',
            staffId: $this->editingStaffId,
            date: $this->editingDate,
            status: $this->editingStatus,
            notes: $this->editingNotes,
        ); // dispatch as browser event, not to a specific Livewire listener
    ...
    public function clearAttendance(): void
    {
        ...
        AttendanceModel::where('monitoring_staff_id', $staffId)
            ->where('date', $date)
            ->delete();
